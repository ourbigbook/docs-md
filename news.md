...
Significant entries will have a corresponding announcement on the following [official accounts](README.md#official-accounts):
...
It is now possible to have create file uploads on [OurBigBook Web](README.md#ourbigbook-web), e.g.:
...
↑ **Parent:** [News](news.md)  
🏷️ **Tags:** [Wikibot](README.md#wikipedia-bot), [Wikibot LLM body population](README.md#wikibot-llm-body-population)
...
When searching articles and topics on [OurBigBook Web PostgreSQL](README.md#ourbigbook-web-postgresql), which is the case for [OurBigBook.com](README.md#ourbigbook-com):
...
We have now cleaned up the tab navigation on [OurBigBook Web](README.md#ourbigbook-web) pages such as:
...
Starting now, if you have a [scoped](README.md#h-scope-argument) header such as `Sample code` in:
```
= x86 Paging
{scope}
...
The [`\H` `disambiguate` argument](README.md#disambiguate-argument) now shows on most navigation locations including:
...
**[Figure 18](#image-ourbigbook-web-tagged-article-list-with-disambiguate). OurBigBook Web tagged article list with disambiguate**. Live URL: [https://ourbigbook.com/go/user/cirosantilli/tagged/classification-mathematics](https://ourbigbook.com/go/user/cirosantilli/tagged/classification-mathematics)
...
Now, only on [`-W`, `--web`](README.md#web), you can also see these article lists with the article content itself, for example:
...
Documentary film
```
under the [split header](README.md#split-headers) page:
```
...
- can't have text under the current header after [Includes](README.md#include). E.g. this is not allowed anymore:
  ```
  = Vertebrate
...
**[Figure 32](#-/333)** The unlisted status is shown as a pill on the article metadata.
...
**[Figure 33](#-/334)** Unlisted articles don't show by default on the topics page, but it is possible to show them by clicking the link at the bottom of the page.
...
**[Figure 34](#-/335)** After that, unlisted articles are also shown.
...
**[Figure 35](#-/336)** A new metadata tab was added to the [web editor](README.md#web-editor).
...
**[Figure 36](#-/337)** The unlisted status can be seen and edited from the newly added metadata tab of the web editor.
...
Previously we would only create an entry in the [`-/file` output directory](README.md#file-output-directory) for [headers](README.md#header) marked wiht the [`\H` `file` argument](README.md#h-file-argument).
...
As a result, when doing a [split header](README.md#split-headers) conversion, it would get both:
- a [`-/file` output directory](README.md#file-output-directory) page at path `_file/file_demo/hello_world.js` [file\_demo/hello\_world.js](README.md#-/file/file_demo/hello_world.js)
- a [`-/raw` directory](README.md#raw-directory) page at path `_raw/file_demo/hello_world.js` [file_demo/hello_world.js](file_demo/hello_world.js)

On the other hand, the test file [file_demo/nofile.js](file_demo/nofile.js) has no such associated header in the source code.

Before this change, [file_demo/nofile.js](file_demo/nofile.js) would only get an [`-/raw` directory](README.md#raw-directory) entry under `_raw/file_demo/nofile.js` and not `_file` entry. But now it also gets both.

The advantages of a `_file` entries over `_raw` entries are as follows:
- `_file` entries can have metadata such as:
  - OurBigBook content associated to them when they have an associated `_file` header. For example at [file\_demo/hello\_world.js](README.md#-/file/file_demo/hello_world.js) we can see the rendered text:> An explanation of what this text file is about.
    > 
    > Another line.
...
Previously, large files with an [`\H` `file` argument](README.md#h-file-argument) associated to them would show a message
```
...
This allows us to now see files that were previously not visible anywhere on the rendered HTML without download due to [`-/raw` files are downloaded rather than displayed in browser for certain file extensions on GitHub Pages](README.md#raw-files-are-downloaded-rather-than-displayed-in-browser-for-certain-file-extensions-on-github-pages).
