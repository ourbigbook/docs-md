...
- by default, topic IDs are automatically singularized, e.g.:
  ...
  ```
  \a[https://ourbigbook.com/-/topic/many-dog][Many Dogs]
  ```
- to prevent this automatic singularization, use [`\x` `p` argument](x-p-argument.md) with `{p}`, e.g.:
  ...
  renders something like:
  ```
  \a[https://ourbigbook.com/-/topic/many-dogs][Many Dogs]
  ```
  ...
  > I like [mathematics](https://ourbigbook.com/-/topic/mathematics)
