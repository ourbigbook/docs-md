...
- start from the most recent header and consider only it and its ancestors in the current branch
- check if the `{parent=XXX}` is a suffix of its ID
- if not, proceed to the next smaller level, and so on, until a suffix is found

Starting a sibling or ancestor-level header closes the previous branch: its descendants can no longer be selected as parents.
