...
The hot-reloading development server stores its output in `web/.next-dev`, while optimized builds and production-mode servers use `web/.next`. This allows frontend builds and build-based tests to run while the development server is running without replacing its chunks. Restart an already running development server once after adopting this configuration.
